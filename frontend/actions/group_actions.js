...
export const fetchGroup = groupId => {
  return dispatch => {
    return GroupApiUtil.fetchGroup(groupId).then(
...
export const fetchGroupsByCategory = category => {
  return dispatch => {
    return GroupApiUtil.fetchGroupsByCategory(category).then(groups =>
...
export const fetchAllGroups = () => {
  return dispatch => {
    return GroupApiUtil.fetchAllGroups().then(groups =>
      dispatch(receiveAllGroups(groups))
    );
  };
};
...
export const createGroup = group => {
  return dispatch => {
    return GroupApiUtil.createGroup(group).then(
      group,
      user => dispatch(receiveGroup(group, user)),
      err => dispatch(receiveErrors(err))
    );
  };
};
...
export const updateGroup = group => {
  return dispatch => {
    return GroupApiUtil.updateGroup(group).then(
      group => dispatch(receiveGroup(group)),
      err => dispatch(receiveErrors(err))
    );
  };
...
export const deleteGroup = groupId => {
  return dispatch => {
    return GroupApiUtil.deleteGroup(groupId).then(
      user => dispatch(receiveGroup({}, user)),
      err => dispatch(receiveErrors(err))
    );
  };
};
...
export const joinGroup = (groupId, userId) => {
  return dispatch => {
    return GroupApiUtil.joinGroup(groupId, userId).then(response =>
      dispatch(receiveJoinedGroup(response.group, response.user))
    );
  };
};
...
export const searchGroups = search => {
  return dispatch => {
    return GroupApiUtil.searchGroups(search).then(groups =>
      dispatch(receiveAllGroups(groups))
    );
  };
};
