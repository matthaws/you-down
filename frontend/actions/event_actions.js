...
export const updateEvent = (event, eventId) => {
  return dispatch => {
    return EventApiUtil.updateEvent(event, eventId).then(
      event => dispatch(receiveEvent(event)),
      err => dispatch(receiveErrors(err))
    );
  };
...
export const joinEvent = (eventId, userId) => {
  return dispatch => {
    return EventApiUtil.joinEvent(eventId, userId).then(
      event => dispatch(receiveEvent(event)),
      err => dispatch(receiveErrors(err))
    );
  };
...
export const leaveEvent = eventId => {
  return dispatch => {
    return EventApiUtil.leaveEvent(eventId).then(event =>
...
export const searchEvents = search => {
  return dispatch => {
    return EventApiUtil.searchEvents(search).then(events =>
      dispatch(receiveAllEvents(events))
    );
  };
};
